#!/usr/bin/env python
"""Post-process PubMed CSV output: filter, rank, and export interactive HTML."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", required=True, help="Input csv path from PubMed pipeline")
...
def resolve_output_paths(input_path: Path, output_dir: str = "") -> tuple[Path, Path, Path]:
    ...
    return out_dir, out_dir / f"{output_base}_ranked.csv", out_dir / f"{output_base}_reading_list.html"
...
def process_pubmed_excel(
    ...
    _, out_csv, out_html = resolve_output_paths(in_path, output_dir)
    if in_path.suffix.lower() != ".csv":
        raise ValueError("CSV input required. Convert xlsx to csv to avoid non-pandas dependencies.")
...
def main() -> None:
    args = parse_args()
    out_csv, out_html = process_pubmed_excel(
