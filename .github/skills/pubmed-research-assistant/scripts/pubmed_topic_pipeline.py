...
from pubmed_fetch_utils import PubmedFetcher
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-csv", default="", help="Optional explicit path for the raw fetched csv")
    parser.add_argument("--raw-xlsx", default="", help="Deprecated: use --raw-csv (csv output only)")
    parser.add_argument("--jcr-path", default="", help="Optional JCR/CSA csv path for IF and quartile annotation")
...
def resolve_raw_csv(raw_csv: str, raw_xlsx: str, run_dir: Path, label: str) -> Path:
    preferred = raw_csv or raw_xlsx
    if preferred:
        path = Path(preferred).expanduser().resolve()
    else:
        path = (run_dir / f"{label}_raw_query.csv").resolve()
    if path.suffix.lower() in {".xls", ".xlsx"}:
        print("Note: raw output uses CSV only. Converting .xlsx path to .csv.")
...
def normalize_publish_date_from_pubmed(raw_csv: Path, api_key: str | None) -> tuple[int, int]:
...
def main() -> None:
    ...
    raw_csv = resolve_raw_csv(args.raw_csv, args.raw_xlsx, run_dir, label)
    ...
    fetcher.get_main_info_into_csv(
        api_key,
        retrieval_query,
        args.release_days,
        args.grab_total,
    ...
    if args.jcr_path:
        jcr_path = Path(args.jcr_path).expanduser().resolve()
        if jcr_path.suffix.lower() != ".csv":
            print("Skip IF/CSA annotation: only CSV JCR/CSA tables are supported.")
        elif jcr_path.exists():
            fetcher.embed_if_into_csv(str(raw_csv), jcr_csa_path=str(jcr_path))
        else:
            print(f"Skip IF/CSA annotation: JCR/CSA file not found: {jcr_path}")
    else:
        skill_root = Path(__file__).resolve().parents[1]
        default_jcr = (skill_root / "references" / "JCR_CSA_2025.csv").resolve()
        if default_jcr.exists():
            fetcher.embed_if_into_csv(str(raw_csv), jcr_csa_path=str(default_jcr))
        else:
            print("Skip IF/CSA annotation: no --jcr-path provided and default JCR file not found.")

    if not args.disable_real_pubdate_fix:
        try:
            checked, replaced = normalize_publish_date_from_pubmed(raw_csv, api_key)
            print(f"publish_date fixed from PubMed ESummary for {replaced}/{checked} PMIDs")
        except Exception as exc:
            print(f"Skip publish_date fix due to error: {exc}")

    ranked_csv, reading_list_html = process_pubmed_excel(
        input_path=raw_csv,
    ...
    if ranked_csv != filtered_csv:
        ranked_csv.replace(filtered_csv)
        ranked_csv = filtered_csv
