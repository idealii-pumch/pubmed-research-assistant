...
Before starting, ensure you have pandas installed and set up your NCBI API key for faster and more reliable PubMed queries:
...
Use the pipeline script when the user wants the full workflow without first producing a local csv.
...
- raw fetched csv when Path A is used (must be unfiltered)
- ranked csv
- HTML reading list
...
- `Use $pubmed-research-assistant to re-rank my existing PubMed csv by keyword and date.`
